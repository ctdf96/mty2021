...
public class JdbcMembersController {
	...
	public String searchMembers(@ModelAttribute MembersSearchModel membersSearchModel,
			Model model) {
	...
	public String registMembers(Model model, @Validated (GroupOrder.class)
	@ModelAttribute MembersModel membersModel, BindingResult result) {
		if(result.hasErrors()) {
			model.addAttribute("headline", "会員登録");
			return "membersRegistration";
		}
		...
		int numberOfRow = membersDao.insert(members);
				if(numberOfRow == 0) {
					...
					model.addAttribute("message", "会員登録");
					return "membersRegistration";
					}
